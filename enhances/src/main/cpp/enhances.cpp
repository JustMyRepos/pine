...
static std::map<ArtMethod, std::vector<std::any>> pending_entries_lollipop_;
...
HOOK_ENTRY(UpdateMethodsCodeLollipop, void, void* thiz, ArtMethod method, const void* quick_code, const void* portable_code, bool have_portable_code) {
    instrumentation_ = thiz;
    if (IsMethodHooked(method)) {
        auto backup = GetMethodBackup(method);
        if (backup) {
...
#define HOOK_SYMBOL(name, symbol, required) hooked |= HookSymbol(handle, symbol, (void*) replace_##name , (void**) &backup_##name , required)
     ...
     if (sdk_level >= __ANDROID_API_N__) {
         ...
         HOOK_SYMBOL(UpdateMethodsCodeLollipop, "_ZN3art15instrumentation15Instrumentation17UpdateMethodsCodeEPNS_6mirror9ArtMethodEPKvS6_b", true);
     } else
         HOOK_SYMBOL(UpdateMethodsCode, "_ZN3art15instrumentation15Instrumentation17UpdateMethodsCodeEPNS_9ArtMethodEPKv", true);
